...
const stack = (() => {
  ...
  const frames = []
  const push = ([signal, fn]) => !signal.queued && frames.push([mark(signal), fn])
  const pop = () => {
    const [signal, fn] = frames.pop()
    return [unmark(signal), fn]
  }
...
Signal.link = (fn, inputs) => {
  ...
  stack.push([self, evaluate]); flush()
  return Signal(self)
}
...
Signal.chain = curry((fn, sa) => {
  ...
  const unlink = ([signal, dependent]) => {
    ...
    filter(signal.dependent, dependent)
  }
...
const set = curry((signal, value) => {
  if (value === undefined) return signal // `undefined` does not propagate
    ...
    signal.value = value
    dependent(signal).reverse().forEach(s => stack.push([s, evaluate]))
