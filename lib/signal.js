...
const stack = (() => {
  ...
  const xs = []
  return {
    push: x => !x.queued && xs.push(mark(x)),
    pop: () => unmark(xs.pop()),
    length: () => xs.length
  }
})()
...
export const Signal = atom => {
  ...
  atom.chain = fn => {
    const linked = ([current, effect]) => current && effect
    const unlink = ([s, dependent]) => filter(s.dependent, dependent)
    const flattened = Signal.of()

    let last, effect
    link(s => {
      when(linked, unlink, [last, effect])
      last = fn(s) // last :: Signal
      effect = isSignal(last) && link(flattened, [last])
    }, [atom])
 ...
 */
const set = (s, value) => {
  if (value === undefined) return s // `undefined` does not propagate
  else if (value === s.value) return s // same value is ignored
  else {
    s.value = value
    dependent(s).reverse().forEach(stack.push)
