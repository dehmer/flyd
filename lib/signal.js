...
export const Signal = atom => {
  ...
  atom.chain = fn => {
    ...
    link(s => {
      ...
      effect = isSignal(signal) && link(self, signal)
    }, [atom])
...
/**
 * scan :: Signal s => (b -> a -> b) -> acc -> s a -> s b
 */
export const scan = curry((fn, acc, signal) =>
 ...
 */
export const tap = curry((fn, signal) =>
 ...
 */
export const loop = curry((fn, acc, signal) =>
  link(a => {
