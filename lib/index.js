...
function updateStream(s) {
  if (hasEnded(s)) return
  if (!isBound(s) && !s.forceUpdate) return;

  if (inStream !== undefined) {
    queueUpdate(updateStream, s);
    return;
  }

  inStream = s;
  if (s.depsChanged) s.fnArgs[s.fnArgs.length - 1] = s.depsChanged;
  const val = s.fn.apply(s.fn, s.fnArgs);
  if (val !== undefined) s(val);
  inStream = undefined;
  ...
  if (listenersNeedUpdating(s)) {
    if (!flushingStreamValue) s(s.val)
      ...
      s.listeners
        .filter(listener => listener.shouldUpdate)
        .forEach(listener => queueUpdate(updateStream, listener))
    }
  }
}

const isBound = s => s.deps.every(s => s.hasVal)
const hasEnded = (s) => s.end && s.end.val === true
const listenersNeedUpdating = s => s.listeners.some(s => s.shouldUpdate)
...
function queueUpdate(updater, s) {
  const noop = () => {}
  s.shouldUpdate = true
  updateQueue.push(() => s.shouldUpdate ? updater(s) : noop())
}
...
function flushUpdate() {
  ...
  while (updateQueue.length > 0) {
    const updater = updateQueue.shift();
    updater()
  }
...
function updateStreamValue(n, s) {
  ...
  if (inStream === undefined) {
    ...
    updateListeners(s);
    if (updateQueue.length > 0) flushUpdate();
    flushingStreamValue = false;
  } else if (inStream === s) {
    // markListeners(s, s.listeners);
    s.listeners.forEach(listener => {
      if (listener.end !== s) {
        if (listener.depsChanged !== undefined) listener.depsChanged.push(s)
        listener.shouldUpdate = true;
      } else {
        endStream(listener)
      }
    })
    ...
    queueUpdate(s => updateStreamValue(n, s), s)
  }
}
